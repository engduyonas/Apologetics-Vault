...
import { getTraditionBySlug, TRADITIONS } from "@/lib/categories";
import CategoryIcon from "@/components/CategoryIcon";
import { BookOpen, Clock, Layers } from "lucide-react";

export function generateStaticParams() {
  return TRADITIONS.map((t) => ({ tradition: t.slug }));
}
...
export default async function TraditionPage({ params }: Props) {
  ...
  const categories = getCategoriesWithCounts(traditionSlug);
  const stats = getTotalStats(traditionSlug);

  return (
    <div className="max-w-6xl mx-auto px-6 lg:px-10 py-12">
      {/* Hero */}
      <div className="text-center mb-14">
        <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-slate-100/50 dark:bg-slate-900/15 text-slate-700 dark:text-slate-400 flex items-center justify-center">
